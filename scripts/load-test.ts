...
// 可选：设置 HTTPS_PROXY/HTTP_PROXY 时，让 undici fetch 统一走该代理（如本机需经代理访问外网）。
...
async function main() {
  ...
  // 网络抖动（代理/线路偶发 ECONNRESET）时按指数退避重试
  async function fetchWithRetry(input: string | URL, init?: RequestInit, retries = 4): Promise<Response> {
    let lastErr: unknown;
  ...
  async function uploadAndCreate(buffer: Buffer, name: string, totalRowsHint: number) {
    ...
    const response = await fetchWithRetry(`${baseUrl}/api/import-tasks`, {
      method: "POST",
  ...
  async function waitForTask(taskId: string, expectedRows: number, timeoutMs = 180000) {
    ...
    while (performance.now() - started < timeoutMs) {
      const response = await fetchWithRetry(`${baseUrl}/api/import-tasks/${taskId}`);
      if (!response.ok) {
        httpErrors++;
