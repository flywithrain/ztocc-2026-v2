...
export default function ImportTaskPage({ params }: { params: Promise<{ taskId: string }> }) {
  ...
  const hasKnownTotal = Boolean(task && task.total_rows > 0 && task.total_batches > 0);
  const percent = useMemo(() => {
    if (!task || task.total_rows <= 0 || task.total_batches <= 0) return 0;
    return Math.min(100, Math.max(0, Math.round((task.processed_rows / task.total_rows) * 100)));
  }, [task]);
  ...
  return <div className="mx-auto max-w-6xl px-4 py-7 sm:px-6">
    ...
    <section className="card import-progress-card mb-5 overflow-hidden border-0">
      <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.2em] text-[#8ec7c5]">Asynchronous Import / Live</p><p className="mt-2 text-4xl font-semibold tracking-tight">{hasKnownTotal ? <>{percent}<span className="ml-1 text-xl text-[#8ec7c5]">%</span></> : <span className="text-2xl">后台解析中</span>}</p><p className="mt-1 text-sm text-[#b6cfce]">{hasKnownTotal ? `${task.processed_rows.toLocaleString()} / ${task.total_rows.toLocaleString()} 行已处理` : "正在读取原文件并生成处理批次"}</p></div><div className="text-right text-sm text-[#b6cfce]"><p>{durationTitle} <strong className="text-white">{durationLabel}</strong></p><p>当前阶段 <strong className="text-white">{stageLabel(task.processing_stage)}</strong></p><p>当前吞吐 <strong className="text-white">{hasKnownTotal ? `${task.throughput.toLocaleString()} 行/分钟` : "—"}</strong></p><p>预计剩余 {hasKnownTotal && task.eta_seconds != null ? formatDuration(task.eta_seconds) : "—"}</p><p>批次 {hasKnownTotal ? `${task.completed_batches} / ${task.total_batches}` : "待生成"}</p></div></div>
      <div className="mt-6 h-2 overflow-hidden rounded-full bg-white/15"><div className={`h-full rounded-full bg-[#70e1cf] transition-all duration-700 ${hasKnownTotal ? "" : "animate-pulse"}`} style={{ width: hasKnownTotal ? `${percent}%` : "28%" }} /></div>
...
function stageLabel(stage: string) {
  return ({
    ...
    parsing: "解析原文件",
    parse_failed: "解析待重试",
    waiting_batches: "等待批次处理",
    dead_lettered: "消息处理失败",
  } as Record<string, string>)[stage] || "批量处理中";
}
