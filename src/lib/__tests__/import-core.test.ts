...
const row = (rowIndex: number, sourceRowNumber = rowIndex + 1): OrderRow => ({ id: crypto.randomUUID(), rowIndex, sourceRowNumber, externalCode: `ORDER_${rowIndex}`, storeName: "测试门店", receiverName: "", receiverPhone: "", receiverAddress: "", skuCode: `SKU_${rowIndex}`, skuName: "测试商品", skuQuantity: 1, skuSpec: "", remark: "" });
...
test("部分行失败不会阻止成功行继续处理", () => { const rows = [row(0), row(1, 8), row(2)]; const result = classifyRows(rows, [{ rowIndex: 1, field: "skuCode", message: "SKU 不存在" }]); assert.deepEqual(result.successfulRows.map((item) => item.rowIndex), [0, 2]); assert.deepEqual(result.failedRows.map((item) => item.rowIndex), [1]); assert.equal(result.failedRows[0].sourceRowNumber, 8); });
